...
CATEGORY_TO_COLLECTION = {'borders': 'borders', 'orders': 'emblems', 'backgrounds': 'backgrounds'}

# order in which groups should appear in each collection's type picker
GROUP_ORDER = {
    'borders': ['Celtic (Gold)', 'Ribbon', 'Rose & Vine', 'Knotwork', 'Scrollwork', 'Ornamental B&W', 'Warlord', 'Side Panels', 'Other'],
    'emblems': ['Crown', 'Dragon', 'Flame', 'Rose', 'Masterhoods', 'Sashes', 'Warrior', 'Other'],
...
def group_for(item):
    coll = CATEGORY_TO_COLLECTION.get(item.get('category'), 'emblems')
...
def main():
    ...
    for coll in ('borders', 'emblems', 'backgrounds'):
        print(coll + ':')
