# Slice & Dice Data Explorer — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a pivot-table data explorer that lets kingdom officers filter, pivot, and drill into Awards, Attendance, and Players data for their chapter.

**Architecture:** Hybrid server/client — a new AJAX controller delivers filtered flat JSON, client-side JS pivots into a row/column matrix, DataTables 1.13.8 renders with drill-down modals. All styling uses existing `.rp-*` CSS classes + new `sd-` prefix classes.

**Tech Stack:** PHP 5.3+ / MySQL (MariaDB), jQuery 1.7.1, DataTables 1.13.8 (CDN), existing `kn-ac-results` autocomplete pattern.

**Spec:** `docs/superpowers/specs/2026-04-16-slice-dice-explorer-design.md`
...
|------|--------|----------------|
| `orkui/controller/controller.Reports.php` | Modify | Add `slicedice()` method — loads kingdom data, sets template |
| `orkui/controller/controller.ReportsAjax.php` | Create | AJAX endpoint: validates params, builds SQL, returns filtered JSON |
| `orkui/model/model.Reports.php` | Modify | Add `get_slicedice_data()` — query builder for each dataset |
| `orkui/template/default/Reports_slicedice.tpl` | Create | Full template: layout, filters, pivot config, DataTable, drill-down modal, all CSS/JS |

---

## Task 1: Controller Route — `Reports/slicedice`

**Files:**
- Modify: `orkui/controller/controller.Reports.php`

This task adds the page route that serves the template with kingdom context data.

- [ ] **Step 1: Add the `slicedice` method to the Reports controller**

Add this method to `controller.Reports.php` after the existing report methods (e.g., after the `knights` method around line 347). The method loads the officer's kingdom data, park list, class list, and kingdom awards for filter dropdowns, then sets the template.
...
Start Docker if not running: `docker-compose -f docker-compose.php8.yml up -d`

Visit: `http://localhost:19080/orkui/index.php?Route=Reports/slicedice`

Expected: A blank page or template-not-found error (since the template doesn't exist yet). The key verification is that the controller method runs without a PHP fatal. Check the browser console / network tab for a 200 response (not a redirect to Login, which would mean auth failed).

- [ ] **Step 3: Commit**

```bash
git add orkui/controller/controller.Reports.php
git commit -m "feat(slicedice): add Reports/slicedice controller route with kingdom data loading"
```

---

## Task 2: AJAX Endpoint — `ReportsAjax/slicedice`

**Files:**
- Create: `orkui/controller/controller.ReportsAjax.php`
- Modify: `orkui/model/model.Reports.php`

This task creates the server-side data endpoint that builds parameterized SQL and returns filtered JSON.

- [ ] **Step 1: Create the ReportsAjax controller**
...
- [ ] **Step 2: Add `get_slicedice_data()` to the Reports model**

Add this method to `orkui/model/model.Reports.php` at the end of the class, before the closing `}`:
...
- [ ] **Step 3: Test the AJAX endpoint**

Open the browser console on any ORK page (while logged in) and run:

```javascript
fetch('/orkui/index.php?Route=ReportsAjax/slicedice', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: 'dataset=players&park_id=0&active_status=all&waiver_status=all'
}).then(r => r.json()).then(d => console.log('count:', d.count, 'sample:', d.records.slice(0,3)));
```

Expected: JSON response with `status: 0`, `count` > 0, and an array of player records with fields: `player`, `player_id`, `given_name`, `park`, `park_id`, `active_status`, `waiver_status`, `year_joined`.

Test each dataset similarly:
```javascript
// Awards
fetch('/orkui/index.php?Route=ReportsAjax/slicedice', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: 'dataset=awards'
}).then(r => r.json()).then(d => console.log('awards:', d.count, d.records.slice(0,2)));

// Attendance
fetch('/orkui/index.php?Route=ReportsAjax/slicedice', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: 'dataset=attendance'
}).then(r => r.json()).then(d => console.log('attendance:', d.count, d.records.slice(0,2)));
```

- [ ] **Step 4: Test filters**

```javascript
// Awards filtered by type
fetch('/orkui/index.php?Route=ReportsAjax/slicedice', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: 'dataset=awards&award_type=ladder'
}).then(r => r.json()).then(d => {
    console.log('ladder awards:', d.count);
    var types = d.records.map(r => r.award_type).filter((v,i,a) => a.indexOf(v) === i);
    console.log('types present:', types); // should be ['ladder'] only
});

// Players filtered by active status
fetch('/orkui/index.php?Route=ReportsAjax/slicedice', {
    method: 'POST',
    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
    body: 'dataset=players&active_status=active'
}).then(r => r.json()).then(d => {
    var statuses = d.records.map(r => r.active_status).filter((v,i,a) => a.indexOf(v) === i);
    console.log('active players:', d.count, 'statuses:', statuses); // should be ['Active'] only
});
```
...
```bash
git add orkui/controller/controller.ReportsAjax.php orkui/model/model.Reports.php
git commit -m "feat(slicedice): add AJAX endpoint with parameterized query builder for awards/attendance/players"
```

---

## Task 3: Template — Page Shell & Dataset Selector

**Files:**
- Create: `orkui/template/default/Reports_slicedice.tpl`

This task creates the template with the page layout, CSS, dataset selector, and placeholder sections for filters/pivot/results. Each subsequent task fills in a section.

- [ ] **Step 1: Create the template with full CSS and page shell**
...
<div class="rp-root">
	...
	<!-- Pivot config (populated by JS after data loads) -->
...
(function() {
	...
	function pivotData(records, rowKey, colKey, aggType) {
		...
		for (var i = 0; i < records.length; i++) {
			var rv = String(records[i][rowKey] ?? 'Unknown');
			var cv = String(records[i][colKey] ?? 'Unknown');
			rowSet[rv] = true;
			colSet[cv] = true;
		}
		...
		for (var i = 0; i < records.length; i++) {
			var rec = records[i];
			var ri = rowIdx[String(rec[rowKey] ?? 'Unknown')];
			var ci = colIdx[String(rec[colKey] ?? 'Unknown')];
	...
	gid('sd-table-wrap').addEventListener('click', function(e) {
		...
		var filtered = currentRecords.filter(function(rec) {
			return String(rec[rowKey] ?? 'Unknown') === rowVal && String(rec[colKey] ?? 'Unknown') === colVal;
		});
...
Expected: The page renders with the header showing "Slice & Dice Explorer" and the kingdom name, three dataset buttons (Awards, Attendance, Players), and filter/pivot sections hidden until a dataset is selected.

- [ ] **Step 3: Commit**

```bash
git add orkui/template/default/Reports_slicedice.tpl
git commit -m "feat(slicedice): add full template with layout, filters, pivot engine, drill-down modal"
```
...
This task verifies the complete end-to-end flow in the browser.

- [ ] **Step 1: Test dataset selection and filter rendering**

Visit `http://localhost:19080/orkui/index.php?Route=Reports/slicedice`

1. Click "Awards" — verify filter bar shows: Park, Award Type, Specific Award, Player search, Date Start, Date End
2. Click "Attendance" — verify filters change to: Park, Class, Type (Event vs Regular), Player search, Date Start, Date End
3. Click "Players" — verify filters change to: Park, Status, Waiver (no player search, no dates)
4. Verify pivot dropdowns update per dataset

- [ ] **Step 2: Test Awards pivot**

1. Select "Awards" dataset
2. Leave all filters at defaults
3. Set Rows = "Park", Columns = "Award Name", Aggregate = "Count"
4. Click "Run Report"
5. Verify: pivot table renders with parks as rows, award names as columns, counts in cells, row/column totals
6. Click a blue number cell — verify drill-down modal opens showing individual award records
7. Close the modal

- [ ] **Step 3: Test Attendance pivot**

1. Select "Attendance" dataset
2. Set Rows = "Park", Columns = "Class", Aggregate = "Count"
3. Click "Run Report"
4. Verify pivot renders
5. Change Aggregate dropdown to "Unique Players" — verify table re-renders instantly (no loading spinner)
6. Change Rows to "Year" — verify table re-renders instantly

- [ ] **Step 4: Test Players pivot**

1. Select "Players" dataset
2. Set Rows = "Park", Columns = "Active/Inactive", Aggregate = "Count"
3. Click "Run Report"
4. Verify pivot renders
...
1. Select "Awards" dataset
2. Set Award Type to "Ladder"
3. Click "Run Report"
4. Click a drill-down cell — verify all records show `award_type: ladder`
5. Set a date range and re-run — verify results narrow

- [ ] **Step 6: Test edge cases**

1. Set very narrow filters that return 0 records — verify "No records match your filters" message
2. Set Row and Column to the same dimension — verify alert "Row and Column dimensions must be different"
3. Test CSV export button works
4. Test Print button works

- [ ] **Step 7: Test player autocomplete (Awards or Attendance)**

1. Select "Awards" dataset
2. In the Player filter, type a known persona (at least 3 chars)
3. Verify dropdown appears with matching players
4. Click a result — verify the text field fills and hidden ID field is set
5. Run Report — verify results are filtered to that player

- [ ] **Step 8: Fix any issues found during testing**

Address any bugs discovered in steps 1-7. Common issues to watch for:
- SQL errors from missing/renamed columns — check browser console for the JSON error response
- Filter dropdowns not populating — verify the PHP controller passes `parks_json`, `classes_json`, `awards_json` correctly
- Pivot renders but values seem wrong — verify the aggregate type matches expectations by drilling down into a cell and manually counting
- Player autocomplete not showing results — verify `KingdomAjax/playersearch/{kingdom_id}` returns data in the network tab

- [ ] **Step 9: Commit fixes**

```bash
git add orkui/template/default/Reports_slicedice.tpl orkui/controller/controller.ReportsAjax.php orkui/model/model.Reports.php
git commit -m "fix(slicedice): address issues found during integration testing"
```

---

## Task 5: Polish & Link from Reports Navigation

**Files:**
- Modify: `orkui/controller/controller.Reports.php` (if there's a reports index/nav that needs a link)
- Modify: `orkui/template/default/Reports_slicedice.tpl` (any polish)

- [ ] **Step 1: Add navigation link**

Check how other reports are linked from the Kingdom and Park profile report tabs. The report should be accessible from the Reports section. Add a link in the appropriate location — likely the kingdom profile's reports tab or the reports index.

Look at how `Reports/knights` is linked and follow the same pattern for `Reports/slicedice/{kingdom_id}`.

- [ ] **Step 2: Final visual polish**

Review the page in the browser and fix any visual issues:
- Ensure the header matches other reports (same padding, font sizes)
- Verify the dataset buttons look good on mobile (may need flex-wrap)
- Verify the pivot table doesn't break horizontally on many columns (scrollX should handle this)
- Verify the drill-down modal looks good and is scrollable for many records

- [ ] **Step 3: Commit**

```bash
git add orkui/controller/controller.Reports.php orkui/template/default/Reports_slicedice.tpl
git commit -m "feat(slicedice): add report navigation link and visual polish"
```
