...
- [ ] **Step 1: Template** — links `reports.css`, `survey.css`, `survey-build.css`; emits `window.SvConfig = { uir: '<?= UIR ?>', surveyId: <?= (int)$SurveyId ?>, survey: <?= json_encode($Survey) ?> }`; markup: `.rp-root > .rp-header` (title editable inline, status pill, actions: Settings, Preview, Open/Close, Results, Copy link, Help) then `.svb-layout` = `.svb-palette` | `.svb-canvas` | `.svb-inspector`. Loads scripts in order: SortableJS, marked, DOMPurify, `survey-render.js`, `survey-build.js`.
- [ ] **Step 2: `survey-build.js`** — single IIFE; state = `SvConfig.survey`; functions `renderCanvas()` (pages as `.svb-page` cards, each question via `SvRender.question(q, undefined, 'preview')` wrapped in `.svb-item` with handle + type badge + required dot), `select(questionId|pageId|'survey')`, `renderInspector()` (per-type settings fields from a `FIELD_DEFS` table keyed by type — every §4 settings key gets a control; options editor with add/remove/reorder/"Other" toggle; show-if pickers listing only earlier `SHOW_IF_SOURCES` questions and their options; markdown toolbar `B I • link image` inserting syntax into the textarea + live preview via `DOMPurify.sanitize(marked.parse(...))`), `save(action, fields)` debounced 400 ms with a `.svb-savestate` pill, `Sortable` on each page's question list (`group:'questions'`, `handle:'.svb-handle'`, `onEnd` → `question_reorder` / `question_move`) and on the page list (`page_reorder`), palette click → `question_add` after the selected item, `image_upload` via `<input type=file>` + FormData, `set_status` with an inline non-native confirm strip for Open/Close, and `locked` handling (disable structural controls, `data-tip` "Locked: this survey has been opened"). Under 900 px: palette becomes a horizontal scroll strip, inspector a bottom sheet toggled by a "Edit" FAB.
