...
**Builder (`Survey/build/{id}`)** — the canvas *is* the editor; there is no side inspector for content. Inside the `.rp-*` frame: a header (inline-editable title, status pill, actions: Settings, Preview, Open/Close, Results, Copy link, Help) and a centred canvas column (max 860 px) of page cards. Clicking a question card selects it (accent left border) and switches it from the `SvRender` preview look to **edit mode, in place**:
...
- **Card footer toolbar** (selected card only): Type `<select>` (every element type), Required toggle, Duplicate, Delete, and a ⋯ menu for show-if (pickers listing only earlier eligible questions and their options), randomize, min/max select, require-all-rows, rank-all, and Add image.
...
- **Autosave** on blur, Enter, or a 400 ms debounce after typing; a `.svb-savestate` pill shows Saved / Saving… / Error. Each edit maps to one AJAX action (`question_update`, `option_set`, `page_update`, `question_add`, `question_reorder`, …). Reordering uses SortableJS on questions (drag handle) and on option rows. When `locked`, add/remove/reorder/type controls are disabled with a `data-tip` explaining why; labels and prompts stay editable.
- **Survey settings** (title, description, welcome/thanks Markdown + image, audience, schedule, data gate, banner, progress, resume, accent) live in a Settings drawer opened from the header, not on the canvas.
- **Density (added 2026-09-10).** The builder is compact in the Google Forms sense; nothing on the canvas is larger than it needs to be. Targets, measured on desktop at 1280 px in both themes:
  ...
  - Prompt: a filled, borderless field (`--ork-surface-light` background, 1 px bottom border) at 16 px text; the small image button sits to its right; the **Type picker is at the top-right of the card**, 34 px tall, icon + label + caret, not in the footer.
  - Option rows: 36 px tall, 20 px control glyph, 14 px label input with only a bottom border on focus, 20 px × on hover/focus; the last row reads "Add option **or** add 'Other'" at 14 px with the links in the accent colour.
  - Footer: 40 px tall, right-aligned: duplicate and delete as 20 px icon buttons, a 1 px divider, "Required" label + a 36×20 px switch, ⋯ menu. No bordered selects or checkboxes in the footer.
  - Typography: 14 px base, 16 px prompt, 12–13 px helper/meta text; inline settings lines (rating end labels, limits) at 13 px with 30 px controls.
  - Settings drawer and list-page controls: 32–34 px inputs and buttons, 12 px labels. Header actions on builder/results/list: 32 px buttons.
  - The runner keeps its 44 px tap targets and 16 px inputs on touch widths (accessibility), but desktop spacing is trimmed to the same 14/16 px type scale.
