...
- **Card footer toolbar** (selected card only): type `<select>`, Required toggle, Duplicate, Delete, and a ⋯ menu for show-if (pickers listing only earlier eligible questions and their options), randomize, min/max select, require-all-rows, rank-all, and Add image.
- **Add** controls: a floating vertical toolbar docked beside the selected card on desktop (question types, section, image, page break); a horizontal strip pinned to the bottom under 900 px.
