...
function initPlayerSearch(cfg) {
	...
	function runSearch(term) {
		...
		$results.innerHTML = '';
		$hidden.value = '';
		/* Filtered: a kingdom (and optionally park) was chosen in the cascade.
		   Run a single scoped search instead of the three-tier global one. */
		if (filterKingdom) {
			var fp = { Action: 'Search/Player', type: 'all', search: term, limit: 25, kingdom_id: filterKingdom };
			if (filterPark) fp.park_id = filterPark;
			$.getJSON(UN_SEARCH_URL, fp).done(function (res) {
				res = res || [];
				if (!res.length) showEmpty();
				else addGroup(filterPark ? 'In Park' : 'In Kingdom', res);
				focusIdx = -1;
				$results.classList.add('un-ac-open');
			});
			return;
		}
		/* Search every player regardless of scope (cross-kingdom / cross-park OK).
		   A generous limit keeps cross-scope matches from being crowded out of the
		   dropdown by alphabetically-earlier local names. Use a "KK: name" or
		   "KK:PP name" prefix to pinpoint a specific kingdom/park. */
		var base = { Action: 'Search/Player', type: 'all', search: term, limit: 25 };
		var calls = [];
