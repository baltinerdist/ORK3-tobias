...
class Controller_Event extends Controller
{
    ...
    public function index($event_id = null)
    {
        ...
        if (!empty($allDetails)) {
            ...
            // Friends attending: the viewer's confirmed-friend ids, for intersecting
            // against each occurrence's RSVP list (login-gated). O(1) lookup map.
            $friendIds = [];
            if ($uid > 0) {
            ...
            foreach ($allDetails as $detail) {
                ...
                // Reuse the manager list when present; otherwise fetch this
                // occurrence's RSVP list solely for the friends intersection
                // (never exposed wholesale to a non-manager).
                $occList = $can_manage ? $this->Event->get_rsvp_list($did) : [];
                $friendsGoing = [];
                if ($uid > 0 && !empty($friendIds)) {
                    $intersectList = $can_manage ? $occList : $this->Event->get_rsvp_list($did);
                    foreach ($intersectList as $att) {
                        if (isset($friendIds[(int)$att['MundaneId']]) && ($att['Status'] ?? '') === 'going') {
                            $friendsGoing[] = [
                                'MundaneId' => (int)$att['MundaneId'],
                                'Persona'   => $att['Persona'],
                            ];
                        }
                    }
                    ...
                    if (!empty($pastAtt)) {
                        ...
                        if ($r['Status'] == 0) {
                            ...
                            if (!$new_detail_id) {
                                ...
                                if ($all) {
                                    $new_detail_id = max(array_map('intval', array_column($all, 'EventCalendarDetailId')));
                                }
                            }
                            if ($new_detail_id) {
                                global $DB;
