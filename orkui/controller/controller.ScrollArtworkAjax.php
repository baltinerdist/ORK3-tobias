...
class Controller_ScrollArtworkAjax extends Controller
{
    ...
    /**
     * Parks the given user may moderate (their session park if they hold
     * AUTH_PARK edit authority there). Returns an array of ids.
     */
    private function moderatable_park_ids($mundane_id)
    {
        $ids = array();
        $pid = isset($this->session->park_id) ? (int)$this->session->park_id : 0;
        if ($pid > 0 && Ork3::$Lib->authorization->HasAuthority($mundane_id, AUTH_PARK, $pid, AUTH_EDIT)) {
            $ids[] = $pid;
        }
        global $DB;
        $DB->Clear();
    ...
    public function upload($id = null)
    {
        ...
        $request = array(
            ...
            'OwnerParkId'       => (int)($_POST['owner_park_id'] ?? 0),
        ...
        // Default OwnerParkId to the submitter's park for the park tier.
        if ($request['Visibility'] === 'park' && $request['OwnerParkId'] <= 0 && isset($this->session->park_id)) {
            $request['OwnerParkId'] = (int)$this->session->park_id;
        }
    ...
    public function browse($id = null)
    {
        ...
        $opts = array(
            ...
            'ViewerParkId'    => isset($this->session->park_id) ? (int)$this->session->park_id : 0,
            'Tier'            => in_array($_GET['tier'] ?? 'all', array('all', 'global', 'kingdom', 'park')) ? $_GET['tier'] : 'all',
     ...
     * queue (default), kingdom officers get their kingdom's queue, park officers
     * get their park's queue. The scope may be forced via the `scope` GET param
     * ('global'|'kingdom'|'park'); a non-admin requesting the global queue gets
     * Status:5.
     *
    ...
    public function pending($id = null)
    {
        ...
        if ($scope === 'global') {
            ...
            $park_ids = $this->moderatable_park_ids($mundane_id);
            if (count($park_ids) === 0) {
                $this->json_response(array('Status' => 5, 'Message' => 'No park moderation authority.'));
            }
            $result = $this->sa->get_pending($page, $per_page, array('Scope' => 'park', 'ParkIds' => $park_ids));
        } else {
    ...
    // ================================================================
    //  POST /ScrollArtworkAjax/update
    // ================================================================

    /**
     * Edit an existing artwork's metadata (and, for user rows, its scope).
     * Tier-aware authorization is handled by the lib (ORK admin for global rows,
     * kingdom/park officers for their scope; re-scoping requires authority on
     * the target scope too). Pack rows: metadata only, scope stays global.
     *
     * POST params: artwork_id, name, description, tags, category_id,
     *              layout_location, visibility, owner_kingdom_id, owner_park_id
     *
     * Returns JSON: {Status, ArtworkId, Message}
     */
    public function update($id = null)
    {
        ...
        // Default the park's kingdom from session when re-scoping to park and the
        // caller did not supply one (park tier stores both park + its kingdom).
        if ($request['Visibility'] === 'park' && $request['OwnerKingdomId'] <= 0 && isset($this->session->kingdom_id)) {
            $request['OwnerKingdomId'] = (int)$this->session->kingdom_id;
        }

        $result = $this->sa->update_metadata($request);

        if (is_array($result['Status']) && isset($result['Status']['Status']) && $result['Status']['Status'] == 0) {
            $this->json_response(array('Status' => 0, 'ArtworkId' => $result['ArtworkId'] ?? $artwork_id, 'Message' => 'Artwork updated.'));
        } else {
            $detail = '';
            if (is_array($result['Status'])) {
                $detail = $result['Status']['Detail'] ?? $result['Status']['Error'] ?? 'Update failed.';
            }
            $this->json_response(array('Status' => 1, 'Message' => $detail));
        }
    }
