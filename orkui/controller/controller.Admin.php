...
class Controller_Admin extends Controller
{
                ...
                switch ($post) {
                    case 'update':
                        $r = $this->Event->update_event(
                            $this->session->token,
                            $event_id,
                            $this->request->Admin_event->KingdomId,
                            $this->request->Admin_event->ParkId,
                            $this->request->Admin_event->MundaneId,
                            $this->request->Admin_event->UnitId,
                            $this->request->Admin_event->Name,
                            strlen($h_imdata) ? $h_imdata : "",
                            $_FILES['Heraldry']['type']
                        );
                        break;
                    case 'edit':
    ...
    public function createevent($post = null)
    {
        ...
        } elseif (valid_id($this->request->MundaneId) || valid_id($this->request->Admin_event->CreateMundaneId)) {
        ...
        if (strlen($action) > 0) {
            ...
            if (!isset($this->session->user_id)) {
                ...
                switch ($action) {
                    ...
                    case 'adddues':
                        if (!valid_id($id)) {
                            $this->data['Error'] = 'Invalid player selection, please try again.';
                            break;
                        }
                        if (!valid_id($this->request->Admin_player->KingdomId)) {
                            $this->data['Error'] = 'Invalid Kingdom, please try again.';
                            break;
                        }
    ...
    public function mergeplayer($params = null)
    {
        ...
        if ('submit' == $params[0]) {
            $post = 'submit';
        } elseif ('park' == $params[0]) {
            $park_id = $params[1];
            $this->data['ParkId'] = $park_id;
            $this->data['KingdomId'] = $this->session->kingdom_id;
        } elseif ('kingdom' == $params[0]) {
            $kingdom_id = $params[1];
            $this->data['KingdomId'] = $kingdom_id;
        }
        $this->load_model('Player');
    ...
    public function createpark($params = null)
    {
        ...
        if (strlen($post) > 0) {
            ...
            } elseif (!valid_id($this->request->Admin_createpark->kingdom_id)) {
                $this->data['Error'] = "Somehow, a Kingdom was not selected.  Good luck with that.";
            } elseif (!valid_id($this->request->Admin_createpark->ParkTitleId)) {
    ...
    public function awards($kingdom_id = null)
    {
        if (empty($this->session->user_id)) {
            header('Location: ' . UIR . 'Login/login/Admin/awards/' . (int)$kingdom_id);
            exit;
        }
        ...
        $this->kingdom_route($kingdom_id);
        $this->load_model('Kingdom');
        $this->load_model('Award');

        // Auth: mirror the existing kingdom-admin award modal gate (CanManageKingdom),
        // i.e. anyone who can manage this kingdom's officers/awards, plus ORK admins.
        // Checked AFTER kingdom_route() so the authorization context is established
        // (same ordering the kingdom() action uses).
        $uid = (int)($this->session->user_id ?? 0);
        $canManageAwards = $uid > 0 && (
            Ork3::$Lib->authorization->HasPermissionOrAuthority($uid, 'kingdom.officer.set', 'kingdom', (int)$kingdom_id, AUTH_CREATE)
            || Ork3::$Lib->authorization->HasAuthority($uid, AUTH_ADMIN, 0, AUTH_ADMIN)
        );
        if (!$canManageAwards) {
            header('Location: ' . UIR . 'Admin/kingdom/' . (int)$kingdom_id);
            exit;
        }

        $kd = $this->Kingdom->get_kingdom_details($kingdom_id);
        // get_kingdom_details bundles an active-only award list we don't use here (we fetch our
        // own list below with ShowDisabled). Drop it so the template can't read stale award data.
        unset($kd['Awards']);
        foreach ($kd as $key => $detail) {
            $this->data[$key] = $detail;
        }
        $this->data['page_title']      = "Awards: " . ($this->data['KingdomInfo']['KingdomName'] ?? '');
        $this->data['IsPrinz']         = $this->data['KingdomInfo']['IsPrincipality'] ?? false;
        $this->data['IsOrkAdmin']      = $uid > 0 && Ork3::$Lib->authorization->HasAuthority($uid, AUTH_ADMIN, 0, AUTH_ADMIN);
        $this->data['CanManageKingdom'] = true;

        // Kingdom awards, INCLUDING disabled, for the management surface
        $adminAwards   = [];
        $awardResult   = $this->Kingdom->GetAwardList([
            'KingdomId'   => $kingdom_id,
            'IsLadder'    => null,
            'IsTitle'     => null,
            'OfficerRole' => 'Awards',
        ...
        if (($awardResult['Status']['Status'] ?? 1) == 0) {
            foreach ($awardResult['Awards'] as $kawId => $aw) {
                $adminAwards[] = [
                    'KingdomAwardId'   => (int)($aw['KingdomAwardId'] ?? $kawId),
                    'KingdomAwardName' => $aw['KingdomAwardName']  ?? '',
                    'AwardId'          => (int)($aw['AwardId']     ?? 0),
                    'AwardName'        => $aw['AwardName']         ?? '',
                    'IsLadder'         => (int)($aw['IsLadder']    ?? 0),
    ...
    public function new_player_attendance()
    {
        ...
        if (!isset($this->request->RunReport)) {
            return;
        }
    ...
    public function auditlog()
    {
        ...
        if (!in_array($entity_type_filter, ['Player', 'Park', 'Kingdom', 'Event', 'Unit'], true)) {
            $entity_type_filter = '';
        }
    ...
    public function ajax($action = null)
    {
        ...
        if ($action === 'suspendplayer') {
            ...
            if (!$from || !$to) {
                echo json_encode(['status' => 1, 'error' => 'Both parks are required.']);
                exit;
            }
            if ($from === $to) {
                echo json_encode(['status' => 1, 'error' => 'Cannot merge a park into itself.']);
                exit;
            }
            ...
            $kingdom_id = (int)($_POST['KingdomId'] ?? 0);
            if (!$park_id || !$kingdom_id) {
                echo json_encode(['status' => 1, 'error' => 'Park and destination kingdom are required.']);
                exit;
            }
            ...
            if (!$from || !$to) {
                echo json_encode(['status' => 1, 'error' => 'Both units are required.']);
                exit;
            }
            if ($from === $to) {
                echo json_encode(['status' => 1, 'error' => 'Cannot merge a unit into itself.']);
                exit;
            }
            $r = $this->Unit->merge([
                'Token'      => $this->session->token,
            ...
            if (!strlen($name)) {
                echo json_encode(['status' => 1, 'error' => 'Kingdom must have a name.']);
                exit;
            }
            if (!strlen($abbr)) {
                echo json_encode(['status' => 1, 'error' => 'Kingdom must have an abbreviation.']);
                exit;
            }
            ...
            if ($fpm_json) {
                ...
                if (!empty($fpm_data['processes']) && is_array($fpm_data['processes'])) {
                    foreach ($fpm_data['processes'] as $proc) {
                        ...
                        if (strpos($uri, '/fpm-status') === 0) {
                            continue;
                        }
                        if (strpos($uri, 'serverhealth_stats') !== false) {
                            continue;
                        }
                        $fpm_workers[] = [
                            'pid'      => (int)($proc['pid'] ?? 0),
    ...
    private function kingdom_route($id)
    {
        ...
        } elseif (!isset($this->session->kingdom_name)) {
            // Direct link
