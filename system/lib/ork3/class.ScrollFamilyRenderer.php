...
class ScrollFamilyRenderer
{
    ...
    public const FONT_DIR = '/var/www/ork.amtgard.com/assets/scroll/fonts/';

    /** Path to families.json (one source of truth). */
    public const FAMILIES_JSON = '/var/www/ork.amtgard.com/orkui/template/revised-frontend/scroll/families.json';
    ...
    public static function families(): array
    {
        if (self::$FAMILIES === null) {
            ...
            if (!file_exists($path)) {
                throw new \RuntimeException("families.json missing: $path");
            }
            self::$FAMILIES = json_decode(file_get_contents($path), true);
            if (!is_array(self::$FAMILIES)) {
                throw new \RuntimeException('families.json failed to decode');
            }
            // Reserved top-level keys (e.g. "_motifs") are manifest metadata, not families.
            self::$FAMILIES = array_filter(self::$FAMILIES, fn ($k) => $k[0] !== '_', ARRAY_FILTER_USE_KEY);
        }
    ...
    public static function renderCanonical($img, int $w, int $h, array $state, array $family): void
    {
        ...
        if (in_array('historiated_initial', $decoration, true) && trim($body) !== '') {
            ...
            self::wrapAndDrawText(
                $img,
                '', // placeholder; the simple line-counter doesn't continue past the indented region
                $bodyX,
                $bodyY + $initialH + (int)(8 * $scale),
                $bodyW,
                $bodyLineH,
                $bodySize,
                $fonts['body'],
        ...
        if (in_array('drolerie', $decoration, true) && isset(self::DROLERIE_KIND[$key])) {
            $drW = max(60, (int)(96 * $scale));
            $drH = max(28, (int)(40 * $scale));
            $drX = (int)($w / 2 + 10 * $scale);
            $drY = $h - (int)(155 * $scale);
    ...
    {
        if (!file_exists($fontPath) || trim($text) === '') {
            return;
        }
        ...
        foreach ($words as $word) {
            ...
            if (($bbox[2] - $bbox[0]) > $maxW) {
                if ($line !== '') {
                    ...
                    $linesDrawn++;
                    if ($lineLimit > 0 && $linesDrawn >= $lineLimit) {
                        return;
                    }
                }
                $y += $lineH;
