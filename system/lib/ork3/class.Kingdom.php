...
class Kingdom extends Ork3
{
    ...
    */
    ...
    public function GetAwardList($request)
    {
        $ladder_clause = '';
        $title_clause  = '';
        if (($request['IsLadder'] ?? null) == 'Ladder') {
            $ladder_clause = " and ka.is_ladder = 1";
        } elseif (($request['IsLadder'] ?? null) == 'NonLadder') {
            $ladder_clause = " and ka.is_ladder = 0";
        }
        if (($request['IsTitle'] ?? null) == 'Title') {
            $title_clause = " and ka.is_title = 1";
        } elseif (($request['IsTitle'] ?? null) == 'NonTitle') {
            $title_clause = " and ka.is_title = 0";
        }
        $disabled_clause = '';
        if (empty($request['ShowDisabled'])) {
    ...
    public function SetAwardDisabled($request)
    {
        if (($mundane_id = Ork3::$Lib->authorization->IsAuthorized($request['Token'])) > 0
                && Ork3::$Lib->authorization->HasPermissionOrAuthority($mundane_id, 'kingdom.award.edit', 'kingdom', $request['KingdomId'], AUTH_CREATE)) {
            $this->log->Write('Award', $mundane_id, LOG_EDIT, $request);
            ...
            $this->kingdomaward->kingdomaward_id = $request['KingdomAwardId'];
            if ($this->kingdomaward->find()) {
                $desired = (int)(bool)$request['Disabled'];
                $this->kingdomaward->disabled = $desired;
                $this->kingdomaward->save();
                // Read-back verify: yapo save() can silently fail (stale PDO binding,
                // constraint) yet still fall through to Success(). Confirm the row's
                // disabled column actually matches what was requested before reporting
                // success, so UI and DB never diverge.
                $this->kingdomaward->clear();
                $this->kingdomaward->kingdom_id = $request['KingdomId'];
                $this->kingdomaward->kingdomaward_id = $request['KingdomAwardId'];
                if ($this->kingdomaward->find() && (int)$this->kingdomaward->disabled === $desired) {
                    return Success();
                }
                return InvalidParameter();
            }
    ...
    public function VacateOfficer($request)
    {
        ...
        if ($mundane_id > 0) {
            if (Ork3::$Lib->authorization->HasPermissionOrAuthority($mundane_id, 'kingdom.officer.vacate', 'kingdom', $request['KingdomId'], AUTH_EDIT)) {
                ...
                $c = new Common();
                $c->set_officer($request['KingdomId'], 0, 0, $request['Role'], 0, $mundane_id);
        ...
        if (($mundane_id = Ork3::$Lib->authorization->IsAuthorized($request['Token'])) > 0
            ...
            $DB->Execute(
                "INSERT INTO " . DB_PREFIX . "officer_history
				 ...
				 VALUES (" . $kid . ", 0, " . $mid . ", '" . $role_esc . "', '" . $start . "', " . $end . ", " . (int)$mundane_id . ", " . $notes . ", NOW())"
            );
            $response = Success();
        } else {
            $response = NoAuthorization();
