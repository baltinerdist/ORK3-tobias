...
class ArtsSciences extends Ork3
{
    ...
    private function annotate_guild_ladders(&$participants, $mundane_ids)
    {
        ...
        $names_sql = "'" . implode("','", array_map('addslashes', $names)) . "'";
        $this->db->Clear();
        ...
        while ($ar && $ar->next()) {
            $nm = (string)$ar->name;
            $aid = (int)$ar->award_id;
            if (isset($letter_for_order[$nm])) {
                $letter_by_award[$aid]  = $letter_for_order[$nm];
            }
            if (isset($letter_for_master[$nm])) {
                $letter_by_master[$aid] = $letter_for_master[$nm];
            }
        }
        if (empty($letter_by_award) && empty($letter_by_master)) {
            return;
        } // schema doesn't have these awards
    ...
    public function SaveAward($request)
    {
        ...
        if (array_key_exists('Rules', $request)) {
            ...
            $this->Award->rules = is_array($rules) ? json_encode($rules) : '';
        }
