...
class Unit extends Ork3
{
    ...
    public function CreateUnit($request)
    {
        ...
        if (($mundane_id = Ork3::$Lib->authorization->IsAuthorized($request['Token'])) > 0) {
            ...
            switch ($this->unit->type) {
                case 'Company': $request['Role'] = 'captain';
                    break;
                case 'Household': $request['Role'] = 'lord';
                    break;
                case 'Event': $request['Role'] = 'organizer';
                    break;
                default: $request['Role'] = 'member';
                    break;
            }
    ...
    private function _active_member_roles($unit_id)
    {
        ...
        if (valid_id($unit_id) && $this->members->find()) {
            do {
                $roles[(int)$this->members->mundane_id] = $this->members->role;
    ...
    public function ClaimUnit($request)
    {
        ...
        Ork3::$Lib->dangeraudit->audit(
            ...
            $unit_id,
            $mundane_id,
            ['unit_id' => $unit_id, 'mundane_id' => $mundane_id]
        );
        return Success();
    }
