...
class Award extends Ork3
{
    ...
    /**
     * Notification trigger: an award/honor was granted to a player.
     *
     * Best-effort, fire-and-forget. Called from the award-grant flow (e.g.
     * Player::AddAward) AFTER the grant has saved successfully. Notifies the
     * RECIPIENT ("You received {AwardName}"). Any failure here is swallowed so
     * the underlying grant save always returns success — never let a
     * notification problem block or fail an award.
     *
     * @param int    $recipientMundaneId recipient's mundane_id
     * @param string $awardName          display name of the award/honor granted
     * @param int    $granterMundaneId   the officer who granted it (audit; 0 = unknown/system)
     * @return void
     */
    public function NotifyAwardGranted($recipientMundaneId, $awardName, $granterMundaneId = 0)
    {
        try {
            $recipientMundaneId = (int) $recipientMundaneId;
            $awardName = trim((string) $awardName);
            if ($recipientMundaneId <= 0 || $awardName === '') {
                return;
            }
            ...
            $fields = [
                'title'    => 'You received ' . $awardName,
                'body'     => 'Congratulations! ' . $awardName . ' has been added to your record.',
                'link_url' => '?Route=Player/profile/' . $recipientMundaneId,
                'payload'  => json_encode(['award_name' => $awardName]),
            ];
            if ((int) $granterMundaneId > 0) {
                $fields['created_by'] = (int) $granterMundaneId;
            }

            Ork3::$Lib->notification->Create($recipientMundaneId, 'award', $fields);
        } catch (\Throwable $e) {
            // Swallow — notifications are best-effort and must never break the grant.
        }
    }

    /**
     * Notification trigger: an award recommendation's status changed
     * (e.g. approved / awarded).
     *
     * Best-effort, fire-and-forget. Called from the recommendation flow AFTER the
     * status change has persisted. Notifies the RECOMMENDER (the mundane who made
     * the rec) that their recommendation moved forward. Any failure is swallowed
     * so the underlying rec save always returns success.
     *
     * @param int    $recommenderMundaneId mundane_id of who made the recommendation
     * @param string $awardName            display name of the recommended award
     * @param string $status               new status, e.g. 'approved' or 'awarded'
     * @param int    $recipientMundaneId   recipient of the rec (link target; 0 = fall back to recommender)
     * @param int    $actorMundaneId       officer who changed the status (audit; 0 = unknown)
     * @return void
     */
    public function NotifyRecStatusChange($recommenderMundaneId, $awardName, $status, $recipientMundaneId = 0, $actorMundaneId = 0)
    {
        try {
            $recommenderMundaneId = (int) $recommenderMundaneId;
            $awardName = trim((string) $awardName);
            $status = strtolower(trim((string) $status));
            if ($recommenderMundaneId <= 0 || $awardName === '') {
                return;
            }
            if (!isset(Ork3::$Lib->notification)) {
                return;
            ...
            // Link target is the recipient's profile (where the rec/award is
            // visible); fall back to the recommender's own profile if unknown.
            $linkMundaneId = (int) $recipientMundaneId > 0 ? (int) $recipientMundaneId : $recommenderMundaneId;

            // Human-readable headline reflecting the new status. Today only
            // 'seconded' is wired (from Player::AddSecondToRecommendation); the
            // 'approved'/'awarded' branches are ready for future rec-status flows.
            if ($status === 'awarded') {
                $title = 'Your recommendation for ' . $awardName . ' was awarded';
                $body  = 'The player you recommended has now received ' . $awardName . '.';
            } elseif ($status === 'approved') {
                $title = 'Your recommendation for ' . $awardName . ' was approved';
                $body  = 'Your recommendation for ' . $awardName . ' has been approved.';
            } elseif ($status === 'seconded') {
                $title = 'Your recommendation for ' . $awardName . ' was seconded';
                $body  = 'Someone added their support to your recommendation for ' . $awardName . '.';
            } else {
                $statusLabel = $status !== '' ? ucfirst($status) : 'updated';
                $title = 'Your recommendation for ' . $awardName . ' was ' . $statusLabel;
                $body  = 'The status of your recommendation for ' . $awardName . ' changed to ' . $statusLabel . '.';
            }

            $fields = [
                'title'    => $title,
                'body'     => $body,
                // Link the recommender back to the recipient's profile, where the
                // recommendation/award is visible.
                'link_url' => '?Route=Player/profile/' . $linkMundaneId,
                'payload'  => json_encode(['award_name' => $awardName, 'status' => $status]),
            ];
            if ((int) $actorMundaneId > 0) {
                $fields['created_by'] = (int) $actorMundaneId;
            }

            Ork3::$Lib->notification->Create($recommenderMundaneId, 'rec_status', $fields);
        } catch (\Throwable $e) {
            // Swallow — notifications are best-effort and must never break the rec save.
        }
    }
}
