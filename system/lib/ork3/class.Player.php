...
class Player extends Ork3
{
    ...
    /**
     * Best-effort display name for an award, for notification copy.
     * Custom name wins; else the award table's name; else a generic label.
     * Never throws.
     */
    private function resolveAwardName($award_id, $custom_name = '')
    {
        ...
        try {
            $this->db->Clear();
            $r = $this->db->query("SELECT name FROM " . DB_PREFIX . "award WHERE award_id = " . $award_id . " LIMIT 1");
            if ($r && $r->size() > 0) {
                $r->next();
    ...
    public function AddAward($request)
    {
        ...
        if (valid_id($mundane_id)
            ...
            // Best-effort recipient notification (never blocks the grant).
            $nAwardName = $this->resolveAwardName($request['AwardId'], $request['CustomName'] ?? '');
            Ork3::$Lib->award->NotifyAwardGranted($request['RecipientId'], $nAwardName, $awards->by_whom_id);
    ...
    public function AddSecondToRecommendation($request)
    {
        ...
        if ($existing->find()) {
            ...
            // Best-effort: tell the recommender their rec was seconded (never blocks the save).
            $nAwardName = $this->resolveAwardName($awardRec->award_id);
            Ork3::$Lib->award->NotifyRecStatusChange($awardRec->recommended_by_id, $nAwardName, 'seconded', $awardRec->mundane_id, $mundane_id);
